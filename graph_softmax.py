...
model = load_model('softmax_network.10.hdf5')
sgd = SGD(lr=0.01, decay=.003, momentum=0.9, nesterov=True)

model_json = model.to_json()
with open("softmax_network.10.json","w") as json_file:
	json_file.write(model_json)

model.save_weights("softmax_network.10.h5")
print("Saved model to disk")


json_file= open("softmax_network.10.json",'r')
loaded_model_json= json_file.read()
json_file.close()
loaded_model = model_from_json(loaded_model_json)
loaded_model.load_weights("softmax_network.10.h5")
...
for i in range(test_size):
	...
	vec = np.clip(vec,0.00001,1)
	entropy_x[i] = -np.sum(vec*np.log(vec))
	entropy_y[i] = -np.sum(y_test[i,:]*np.log(vec))
